...
(defn handle-reservation' [{:keys [reservation-channel] :as system}]
  (lib.async/serialise-calls reservation-channel (handle-reservation system)))
...
(defn -main [& _args]
  ...
  (let [server (system/start {:server                  {:port 3000}
                              ...
                              :reservation-channel     {:port    (async/chan 1024)
