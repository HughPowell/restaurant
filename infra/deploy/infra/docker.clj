...
(defn- create-image* [client name]
  (letfn [(existing-image [client name] (containers/invoke client {:op                   :ImageInspect
                                                                   :params               {:name name}
...
(defn create-image [component]
  [{:enter (fn [{{{:keys [images]} :clients
                  {:keys [image]}  component} :request
                 :as                         ctx}]
             (create-image* images image)
             ctx)}])
